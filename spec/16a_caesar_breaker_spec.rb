...
describe CaesarBreaker do
  ...
  describe '#save_decrypted_messages' do
    ...
    context 'when the directory does not exist' do
      ...
      it 'sends message to check the existance of the 16_cipher directory' do
        expect(Dir).to receive(:exist?).with('16_cipher').and_return false
  ...
  describe '#save_to_yaml' do
    # Method with Outgoing Command -> Test that a message is sent
    context "Tests whether the info is being dumped" do
      before do
      end
    
    it 'dumps to yaml' do
      expect(YAML).to receive(:dump).exactly(1).times
