...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        # allow(input).to receive(:gets).and_return("100")
        allow(game_input).to receive(:gets).and_return("100", "3")
      end
      ...
      it 'completes loop and displays error message once' do
        ...
        max = game_input.instance_variable_get(:@maximum)
        error_message = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input).to receive(:puts).and_return(error_message)
        game_input.player_input(min, max)
      end
    end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return("100", "200", "3")
      end
  ...
  describe '#verify_input' do
    ...
    subject(:test_input) { described_class.new(3, 21) }

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = test_input.instance_variable_get(:@minimum)
        max = test_input.instance_variable_get(:@maximum)
        expect(test_input.verify_input(min, max, 5)).to be(5) 
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        min = test_input.instance_variable_get(:@minimum)
        max = test_input.instance_variable_get(:@maximum)
        expect(test_input.verify_input(min, max, 300)).to be_nil 
      end
    end
  end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      ...
      it 'returns 9' do
        expect(game_six_hundred.maximum_guesses).to be(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    before do
      ...
      allow(game).to receive(:display_guess).with(binary_search).and_return(0)
      allow(binary_search).to receive(:update_range).and_return(0)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game.display_turn_order(binary_search) }
      .to change { game.instance_variable_get(:@guess_count) }.by(1)
    end
