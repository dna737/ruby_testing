...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:unfinished_game) { described_class.new(5, '4') }
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        false_input = 'a'
        valid_input = '7'
        allow(game_loop).to receive(:player_input).and_return(false_input, false_input,valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:hard_game) {described_class.new(7, '7', 5)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        final_output = "That was hard. It took you 5 guesses!\n"
        expect { hard_game.final_message }.to output(final_output).to_stdout
      end
    end
  end
end
