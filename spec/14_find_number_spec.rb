...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let (:random_number) {double('answer', value: 5)}
      subject(:end_game) {described_class.new(2,7,random_number, 5)}
    ...
    context 'when guess and random_number are not equal' do
      ...
      subject(:not_finished) {described_class.new(2, 6, random_number, 10)}
      it 'is not game over' do
        expect(not_finished).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        expect { low_guess_game.update_range }
        .to change { low_guess_game.min }.to(5)
      end

      it 'does not update max' do
        expect { low_guess_game.update_range }
        .to change { low_guess_game.max }.by(0)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect { edge_case.update_range }.to change { edge_case.min }.to(edge_case.max)
      end

      it 'does not update max' do
        expect { edge_case.update_range }.to change { edge_case.max }.by(0)
      end
    end
  end
end
